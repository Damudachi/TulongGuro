...
| rubric | text (JSON) | NULL | The rubric governing this activity, embedded as JSON. First of only two grading tiers, both of which must be human-written: this rubric, then the curriculum lesson's rubric. There is deliberately no third tier. An earlier build fell through to a recommended template and then to a hardcoded generic rubric, which meant an activity with no rubric was never refused but was graded against criteria nobody in the school had written; AI checking now refuses outright. |
