 ...
 *
 * Strength is not the same question as acceptability. `Passwor1` clears the
 * rule and is still poor; the meter says so without blocking it, because a
 * meter that refuses is just a rule with worse manners.
 *
 * score is 0-4 and drives the bar. Anything failing passwordProblem() is 0.
 */
function passwordStrength(password) {
  ...
  // Past the rule, length is what actually buys resistance to guessing, so it
  // is worth two of the four points on its own. Variety is the rest.
  let score = 1;
  if (value.length >= 12) score += 1;
  if (value.length >= 16) score += 1;
  if (/[^A-Za-z0-9]/.test(value)) score += 1;
  // A password made of one repeated character or a straight run ("aaaaaaaA1",
  // "abcdefG1") passes every class check and is still trivially guessed.
